...
func main() {
	...
	header, status, wsClient, channels, restClient, teamID, channelsWidth, showModeIndicator, activeHeaderFg, activeHeaderBg, fullDateFormat := loadStartupState(resolvedConfig)
	if wsClient != nil {
		wsClient.Start(ctx)
	}
	...
	m := tui.NewModelWithHeader(header, status, eventsCh, statusCh, channels, st, restClient, teamID, channelsWidth, showModeIndicator, activeHeaderFg, activeHeaderBg, fullDateFormat)
...
// and the full date format for messages not from today.
func loadStartupState(path string) (tui.HeaderInfo, string, *mattermost.WSClient, []mattermost.Channel, *mattermost.Client, string, int, bool, string, string, string) {
	header := tui.HeaderInfo{Status: mattermost.ConnStatusConnecting}

	if _, err := os.Stat(path); os.IsNotExist(err) {
		return header, "Config file not found. Run with --config path/to/config.toml", nil, nil, nil, "", 22, true, "15", "237", "02.01.2006"
	}
	...
	return header, "", wsClient, channels, client, teamID, cfg.UI.ChannelsWidth, cfg.UI.ShowModeIndicator, cfg.Colors.ActiveHeaderFg, cfg.Colors.ActiveHeaderBg, cfg.UI.FullDateFormat
}
