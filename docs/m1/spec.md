...
Нужно улучшить клавиатурную навигацию. Сейчас она была сделана по ходу реализации, нужно всё довести до удобной навигации.

**Что делаем:**
Окно разделено на несколько активных частей, сейчас их 2: messages, input. В будущем добавим еще список каналов (channels) слева.
...
- ctrl+j - переход в channels (когда будет)
- ctrl+f - перехов в search_message (когда будет)
- ctrl+k - search_channel (когда будет)
- esc всегда возращает в input

По умолчанию пользователь находится в input:
- Тут можно вводить текст, команды, клавиатурная навигация стрелками позволяет двигать курсор
- Отправка команды по cmd+enter, просто по enter вставляется перенос строки
- ctrl+c очищяет поле
- esc: первое нажатие пишет в статусе что нужно нажать еще раз esc чтобы очистить поле, после второго нажатия поле очизается, второе нажатие ждем 3 секунды
- для удобства - когда input пустое и пользователь нажимает up, то переходим в messages

Когда пользователь находится в messages:
- текущее выбранное сообщение подсвечивается светло-серым фоном
- по умолчанию выбрано последнее сообщение снизу
- если нажимать up/down - выполняется навигация вверх вниз
- если пользователь нажимает / (slash) то slash вставляется в input и мы переходим в input для ввода команды, в этом случае сообщение остается **выбранным**
- (сделаем позже) по горящим клавишам будут выполняться действия с сообщениями
	- e - (только если текущий юзер - автор) редактирование, вставляется команда '/edit {текст сообщения}' и переход в input
	- d - (только если текущий юзер - автор) откроет popup удаления сообщения
	- r - вставляется команда '/reply' и переход в input
- (сделаем позже) по enter сообщение будет открываться в popup (thread)

Механизм выбранных сообщений:
- если пользователь выбирает сообщение и затем переходит в input через / (slash), ctrl+b или по горячей клавише, то сообщение остается выбранным, подсвеченным
- в status bar видно что выбрано сообщение и id сообщения серым цветом, затем надпись (esc twice to unselect)
- если нажать esc дважды, то поле очищается и сообщение убирается
- если нажать ctrl+c одинажны,  то поле очищается и сообщение убирается
