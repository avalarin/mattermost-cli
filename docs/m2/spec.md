...
- Навигация по панелям — только горячие клавиши (Tab/Shift+Tab убраны):
  - `Ctrl+B` → input (сохраняется из M1)
...
*`views/thread.go` — `ThreadView`:*
- `viewport.Model` внутри
- Показывает root-сообщение выделенным + все replies
- Заголовок: «Thread»
- ↑/↓ / PgUp/PgDn прокрутка
...
*`model.go`:*
- `threadView *ThreadView` — `nil` когда закрыта
- `openThreadID string` — ID root-сообщения открытого треда
- Enter на сообщении в ModeMessages → если тред другого сообщения — переключаем; если треда нет — открываем; фокус переходит в ModeThread
- Esc из ModeThread → закрываем threadView, переходим в ModeMessages
- `Ctrl+B` из ModeThread → переходим в ModeInput (тред остаётся открытым)
- В ModeThread горячие клавиши на выбранном сообщении:
  - `r` → вставляет `/reply` в input + ModeInput
  - `e` → (если `msg.UserID == currentUserID`) вставляет `/edit <текст>` в input + ModeInput
  - `d` → (если `msg.UserID == currentUserID`) открывает popup подтверждения удаления (реализация popup — заглушка, функциональность в M3)
- Layout: при `threadView != nil` → 3 колонки `[channels | messages | thread]`
  - Ширина thread = `ui.thread_width` (default 40)
  - Ширина messages = ширина - channels - thread - разделители

**Критерии приемки:**
- Enter на сообщении → справа появляется панель треда с ответами; фокус в треде
- Esc из треда → панель закрывается, фокус в messages
- Содержимое треда прокручивается независимо от messages
- `r` вставляет `/reply` в input и переключает в input
- `e` на своём сообщении вставляет `/edit <текст>` в input
- Ctrl+B из треда → input (тред не закрывается)

**Как проверить руками:**
1. Выбери сообщение с ответами (`⤵︎ N`), нажми Enter → справа появляется панель треда
2. В панели треда видны root + все ответы; курсор на последнем
3. ↑/↓ — навигация по сообщениям треда
4. Нажми `r` → input заполняется `/reply`, фокус в input
5. Esc из треда → панель закрывается, 2 колонки
6. Ctrl+B из треда → фокус в input, тред остаётся видимым
...
| 2 | `TestEnterOpensThread` | Enter на сообщении → threadView != nil, ModeThread |
| 3 | `TestEscClosesThread` | Esc из ModeThread → threadView == nil, ModeMessages |
| 4 | `TestCtrlBFromThreadGoesToInput` | Ctrl+B из ModeThread → ModeInput, threadView != nil |
| 5 | `TestThreadRKeyInsertsReply` | `r` в ModeThread → input содержит "/reply", ModeInput |
| 6 | `TestThreadLayout3Columns` | При открытом треде View() содержит 3 колонки |
...
- При скролле до дна MessagesView активного канала → `markReadCmd(channelID)` (fire-and-forget)
...
- При прокрутке до дна в канале — счётчик сбрасывается
...
3. Перейди в тот канал, прокрути до дна → счётчик исчезает
...
| 4 | `TestUnreadClearsOnScrollBottom` | Скролл до дна активного канала → markReadCmd запускается |
| 5 | `TestChannelsViewRendersUnreadBadge` | ChannelsView рендерит `#general (3)` при count>0 |

---

### T6: Поиск каналов (popup)

**Что делаем:**

*Новый `views/channel_search.go` — `ChannelSearchPopup`:*
- Popup-диалог по центру экрана (поверх основного контента)
- Ширина: 60 символов (или ширина экрана −4, если меньше)
- Структура:
  ```
  ┌─ Channel Search ──────────────────────────────┐
  │ > query_                                      │
  ...
  │   @alice                                      │
  ├───────────────────────────────────────────────┤
  │ ↑↓ navigate · Enter open · Esc/Ctrl+C close   │
  └───────────────────────────────────────────────┘
  ```
- Поле ввода вверху; список результатов по центру; хоткеи внизу
- Фильтрация case-insensitive по Name и DisplayName; «All Activity» всегда в начале списка

*Новый `ModeSearch` или `searchActive bool` в `model.go`:*
- `Ctrl+K` (из любого режима) → открывает popup (`searchView != nil`), устанавливает фокус
- Символы → фильтруют список; `↑`/`↓` → навигация по результатам
- `Enter` → открывает выбранный канал (MsgChannelSelected), канал становится «выбранным и открытым» в channels panel; popup закрывается
- `Esc` или `Ctrl+C` → закрывает popup без изменений
- `Backspace` → удаляет последний символ из запроса
...
- Ввод фильтрует список в реальном времени
- «All Activity» всегда в начале
- Enter открывает канал, channels panel подсвечивает его как открытый
- Esc и Ctrl+C закрывают popup
- Внизу popup видны хоткеи

**Как проверить руками:**
1. Нажми `Ctrl+K` → popup открывается по центру
2. Введи `gen` → список фильтруется
3. `↑`/`↓` → навигация по результатам
4. Enter → канал открывается, popup закрывается, channels panel показывает его открытым
5. Снова `Ctrl+K`, Esc → popup закрывается, ничего не изменилось

**Сценарии автотестов:**

| # | Название | Что проверяем |
|---|----------|---------------|
| 1 | `TestCtrlKOpensSearchPopup` | Ctrl+K → searchView != nil |
| 2 | `TestSearchPopupFiltersChannels` | Ввод "gen" → результаты содержат только matching каналы |
| 3 | `TestSearchPopupAlwaysShowsAllActivity` | «All Activity» всегда первым |
| 4 | `TestSearchPopupEscCloses` | Esc → searchView == nil |
| 5 | `TestSearchPopupEnterOpensChannel` | Enter → MsgChannelSelected, searchView == nil |
| 6 | `TestSearchPopupCtrlCCloses` | Ctrl+C → searchView == nil |
...
thread_width   = ui.thread_width     (default: 40, только при открытом треде)
dividers       = 2                   (вертикальные разделители)

messages_width = total - channels_width - dividers
               = total - channels_width - thread_width - dividers  (с тредом)
```
...
|---|---|---|
| Ctrl+B | любой | → ModeInput |
| Ctrl+J | любой | → ModeMessages (сохраняется из M1) |
| Ctrl+L | любой | → ModeChannels (новое) |
| Ctrl+K | любой | открыть channel search popup |
| Esc | любой | → ModeInput |
...
| Enter | ModeMessages | открыть тред |
...
| d | ModeThread (автор) | открыть popup удаления |
| Esc | ModeThread | закрыть тред → ModeMessages |
| Ctrl+B | ModeThread | → ModeInput (тред остаётся) |
| ↑ / ↓ | ModeSearch | навигация по результатам |
| Enter | ModeSearch | открыть канал, закрыть popup |
