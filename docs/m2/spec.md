...
- Popup показывает: название, описание, список участников (до 10 строк, скролл ↑↓)
- `Esc` закрывает popup и возвращает фокус в channels
- `Enter` на выбранном участнике — открывает DM с ним и закрывает popup
- Если описание пустое — показывается «No description»; участники загружаются из REST асинхронно (до загрузки — «Loading...»)
...
DEBUG:
- Для того чтобы удобнее тестировать нужно добавить debug фильтр deleted channels, он по умолчанию скрыт, появляется если в конфиге включить в разделе `[debug]` опцию

**Критерии приемки:**
- Архивированные каналы всегда видны в списке с маркером `[x]` (API включает `include_deleted=true`)
- Архивированный канал можно открыть и читать как обычный
- Фильтр «Archived only» в Ctrl+K popup показывает только архивные каналы (`ArchivedOnly` в `ChannelFilterState`)
- `Esc` в Ctrl+K popup — отменяет изменения фильтра; `Enter` в секции фильтров — применяет

**Как проверить руками:**
1. Если в MM есть архивированный канал → он виден с `[x]` в боковой панели
2. Enter на нём → открывается история как у обычного канала
3. Ctrl+K → включить «Archived» → список покажет только архивные каналы
4. Esc → фильтр не применяется; Enter в секции фильтров → применяется
...
| 2 | `TestArchivedChannelEnterOpens` | Enter на архивированном → MsgChannelSelected (не ошибка) |
...
**Критерии приемки:**
- `i` на канале → popup с названием, описанием, списком участников (до 10 строк)
- Участники загружаются из REST асинхронно; до загрузки — «Loading...»
- ↑/↓ навигация по списку участников; выбранный выделен
- Enter на участнике → открывает DM, popup закрывается
- Esc → popup закрывается, фокус обратно в channels

**Как проверить руками:**
1. Ctrl+L → channels, выбери канал, нажми `i` → popup открывается
2. Дождись загрузки участников, навигируй ↑/↓
3. Enter на участнике → открывается DM с ним
4. Esc → popup закрывается, фокус в channels
...
| 2 | `TestInfoKeyOpensPopup` | `i` в ModeChannels → infoPopup != nil, ModeInfo |
| 3 | `TestInfoPopupEscCloses` | Esc → infoPopup == nil, ModeChannels |
| 4 | `TestInfoPopupEnterCloses` | Enter (без выбранного участника) → infoPopup == nil, ModeChannels |
