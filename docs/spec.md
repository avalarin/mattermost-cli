...
**Функциональность:**
- Левая панель: список каналов с бейджем непрочитанных (`#general (3)`). Счётчик непрочитанных — с сервера при старте, далее поддерживается через WS.
- Правая панель: сообщения выбранного канала. Автоматическая подгрузка истории при прокрутке вверх.
- Канал помечается прочитанным, когда пользователь прокручивает ленту до последнего сообщения.
- В главной панели — только root-сообщения. Если у сообщения есть ответы — бейдж `↩ 3`.
...
| Клавиша | Действие |
|---|---|
| `Tab` / `Shift+Tab` | Переключение фокуса между панелями (каналы → сообщения → тред) |
...
| `Esc` | Закрыть тред / выйти из поиска |
