...
func testModelWithClient(t *testing.T, client *mattermost.Client, teamID string) Model {
	t.Helper()
	m := NewModelWithHeader(HeaderInfo{}, "", nil, nil, nil, nil, client, teamID, 22, false, "15", "237", "", "root_only", 70, "alphabetical", false, false)
	updated, _ := m.Update(tea.WindowSizeMsg{Width: 80, Height: 24})
	return mustModel(t, updated)
}
...
func TestHelpPopupOpens(t *testing.T) {
	m := NewModelWithHeader(HeaderInfo{}, "", nil, nil, nil, nil, nil, "", 22, false, "15", "237", "", "root_only", 70, "alphabetical", false, false)
	m = initModel(t, m)
...
func TestMessagesViewShowAll(t *testing.T) {
	m := NewModelWithHeader(HeaderInfo{}, "", nil, nil, nil, nil, nil, "", 22, false, "15", "237", "", "all", 70, "alphabetical", false, false)
	m = initModel(t, m)
	m.activeChannelID = "chan1"
...
// using max(local, server) so neither WS-incremented counts nor server history is lost.
func TestUnreadsLoadedUpdatesModel(t *testing.T) {
	...
	// ch2: WS says 2, server says 5 — take the higher value to preserve server history.
	if m.unreadCounts["ch2"] != 5 {
		t.Errorf("unreadCounts[ch2] = %d, want 5 (max of WS=2 and server=5)", m.unreadCounts["ch2"])
	}
